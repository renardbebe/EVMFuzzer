...
class Chain(BaseChain):
    ...
    def get_transaction_result(
        ...
        with self.get_vm(at_header).state_in_temp_block() as state:
            # set sender balance
            if setBalance :
                state.account_db.set_balance(transaction.sender, setBalance)
...
class MiningChain(Chain):
    ...
    def apply_transaction(self, transaction, setBalance=0):
        """
        ...
        # set sender balance
        if setBalance :
            vm.state.account_db.set_balance(transaction.sender, setBalance)
        # print(vm.state.account_db.get_balance(transaction.sender))
